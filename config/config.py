...
class DATASET:
    ...
    # Path to Val set and Test set
    VAL_SET = os.path.join(ROOT, NAME, 'test')  # If it exists
...
class CKPT:
    ...
    # SAVE_DIR = os.path.join('outputs', 'weights', '{}_{}'.format(MODEL.NAME, 'DUTS'))
    SELECTED_INFER_CKPT = '60-ckpt.pth'
...
class TEST:
    COMPLETE_TEST_IMAGE_DIR = os.path.join(DATASET.ROOT, DATASET.NAME, 'image', 'test')
